/**
 * My comments will have CR at the beinning of them. They will also look like this. 
 */
...
/**
 * 
 * It looks like this function is adding "here" to one div in a list of div and adding "displaynone" to everything else.
 * 
 * If so, look at #2 for a way to do this a little better.
 */
//function that makes divs visble
  ...
  function createPhotoDiv(backgroundUrl1, rightOrLeft){
        ...
        // event should be a parameter CR
        function changeActiveMajorDivHeaderAndColor(event){
...
//connecting the actions of making divs visible and changing the backgrounds with their selected buttons
  document.querySelector('#gallerybutton').addEventListener("click", 
        ...
        // If the bottom five things represent "one action" like "edit the styles", make it a function) CR #1
        // and replace.
...
//come back to- h3 on contact not working                
          ...
          } // It would make sense to put an else {} here containing anything that would only happen in mobile, even though there's
          // only two options.

          // If Mobile is a "stripped down version" of the desktop popup, put make sure it's totally loaded for mobile before 
...
// Should this function be running on ever resize (the resize listener runs every time the browser moves like a milimeter).
// Adding any listeners or styles happens over and over and over again in one resize.
